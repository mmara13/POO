...
public:
    Track(const T& trackData):data(trackData){} //constructor initializare
...
template<typename T>
void printInfo(const T& data){
    cout<<"Information about data provided: "<<typeid(T).name();
    cout<<"\n";
}

template<>
void Track<Song>::play() const{
    cout<<"Playing: "<<data.getSongTitle()<<" by "<<data.getSongArtist().getArtistNickname()<<"\n";
}

template<>
void Track<Album>::play() const{
    cout<<"Playing album: "<<data.getAlbumTitle()<<" by "<<data.getAlbumArtist().getArtistNickname()<<"\n";
}

template<>
void Track<Playlist>::play() const{
    cout<<"Playing playlist: "<<data.getPlaylistTitle()<<"\n";
}
...
class SongIterator{
    ...
    int index;
public: 
    SongIterator(const vector<Song>& s):songs(s), index(0){}
...
try{
    ...
    myUser.authenticateUser("mara13", "parola12345");

    //partea 3
    Track<Playlist> track1(muzica);
    Track<Album> track2(album_the_weeknd);
    Track<Song> track3(tz1);

    track1.play();
    track2.play();
    track3.play();
    Track<Mixtape> track4(mixtape1);
    track4.play(); //afiseaza Now playing ptc e data necun

    printInfo(track3);

    //cerinta design patterns OOP
    //1 Composite pattern: clasa Media e clasa de baza, iar Song, Album si Playlist sunt clase derivate
    //ele contin vector de pointeri la media, formand structura ierarhica

    //2 Iterator Pattern: in albume/playlist-uri avem vectori de melodii
    //permite parcurgerea elementelor intr-o colectie fara a cunoaste implementarea colectiei
 
    //implementare iterator pattern
    const vector<Song>& album_songs = album_the_weeknd.getAlbumSongs();
    SongIterator iterator(album_songs);
    while(iterator.hasNext()){
        const Song& s = iterator.next();
        cout<<s.getSongTitle()<<'\n';
    }
